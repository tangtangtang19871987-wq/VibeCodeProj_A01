...
| X-16 | X-14's optimizer swap extended to all 10 ICCAD13 cases, G-015 | [C] | `configs/experiments/iccad13_ilt_lbfgs.yaml` | `results/iccad13_ilt_lbfgs/` | **VERIF** -- L-BFGS matches the paper's Table 1 "OURS" EPE@15nm column EXACTLY on 10/10 cases (mean 1.6 == 1.6); closes both the main baseline's case-3 gap (22->13) and case-5's F-PERF-01 pixel (1->0) as a side effect (F-LBFGS-02) |
| X-17 | Three designed directions for improving X-16 (objective-shaping, multi-resolution, multi-start); the iteration-budget effect they surfaced, extended to all 10 cases, G-015 | [C] | `scripts/ilt_improvement_directions.py`, `configs/experiments/iccad13_ilt_lbfgs100.yaml` | `results/ilt_improvement_directions/`, `results/iccad13_ilt_lbfgs100/` | **VERIF** -- all 3 designed directions tested (D1 backfired, D2's hypothesis refuted by its own control, D3 marginal at 4x cost); the emergent fix (iterations 50->100) gives mean EPE@15nm=1.0, BETTER than the paper's own 1.6, and cuts mean EPE@3nm 55.5->46.6 (still short of the paper's 32.8 ISPD25 reference) (F-LBFGS-03) |
